...
def image_analysis_prompt(prompt, description):
    ...
    if not separator:
        system, user = '', prompt
    system += ('分析图片问题时仍以面试者本人身份直接作答，不添加面试指导或与题目无关的说明。下方图片识别结果由 Flash 从原图提取；'
               '你接收的是识别结果而非原始图片，请据此回答原始请求，不要声称亲自看过原图，'
               '不主动介绍 Flash、Pro、OCR 或图片处理流程；'
        ...
        async def request():
            ...
            if references:
                system += ('用户已授权只读查阅以下参考资料。问题涉及资料中的项目或具体内容时，先搜索或读取相关资料再回答；'
                    ...
                    '面试作答时将相关事实自然融入第一人称回答，不展示资料路径、行号、页码或读取过程；'
                    '仅在当前问题明确要求来源、引用或文件查证时，注明实际工具返回的路径与行号，PDF 还可注明页码。'
