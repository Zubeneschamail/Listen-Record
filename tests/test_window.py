...
class WindowFlowTests(unittest.TestCase):
    ...
    def test_settings_dialog_preserves_layout_and_values(self):
        ...
        try:
            ...
            self.assertTrue(app.settings_window.overrideredirect())
            ...
            self.assertEqual(app.hotwords.get(), "大模型,闻录")
            app.settings_tabs.select(app.settings_pages['tools'])
            root.update()
            self.assertEqual(app.settings_window.winfo_height(), 440)
            ...
            with patch.object(app, 'open_folder') as folder:
                app.settings_actions['历史记录'].invoke()
    ...
    def test_ctrl_click_combines_nonadjacent_rows_in_one_request(self):
        ...
        try:
            with tempfile.TemporaryDirectory() as folder:
                ...
                click_row(2)
                self.assertEqual(app.multi_rows, {2})
